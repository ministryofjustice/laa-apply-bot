  ...
  class TestRunMonitor
    ...
    def perform(monitor_url, delay, channel, web_url, message_ts)
      ...
      if result["status"].eql?("completed")
        block = Slack::BlockBuilder.call(:complete, result: result["conclusion"].eql?("success"), web_url:)
        SendSlackMessage.new.update({ ts: message_ts, channel:, as_user: true }.merge(block))
      else
